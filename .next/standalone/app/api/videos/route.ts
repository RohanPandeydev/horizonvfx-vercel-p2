  ...
  try {
    ...
    const videosWithParsedTech = await Promise.all(
      videos.map(async (video) => ({
        ...
        // Generate URLs for serving files
        thumbnailUrl: await getFileUrl(video.thumbnailS3Key),
        videoUrl: video.videoS3Key ? await getFileUrl(video.videoS3Key) : null,
      }))
    );
...
export async function POST(request: NextRequest) {
  try {
    ...
    const thumbnailS3Key = generateFileKey(thumbnail.name);
    await uploadFile(thumbnailBuffer, thumbnailS3Key, thumbnail.type);

    // Upload video
    const videoBuffer = Buffer.from(await video.arrayBuffer());
    const videoS3Key = generateFileKey(video.name);
    await uploadFile(videoBuffer, videoS3Key, video.type);

    // Generate URLs for serving files
    const thumbnailUrl = await getFileUrl(thumbnailS3Key);
    const videoUrl = await getFileUrl(videoS3Key);
